...
const state = reactive({
  form: { id: '', consecutiveDay: 1, rewardPoint: 1, sortOrder: 0, status: '0' },
});

const rules = reactive({
  consecutiveDay: [{ required: true, message: '请输入连续签到天数', trigger: 'blur' }],
...
const initForm = async (row?: any) => {
  ...
  if (row?.id) {
    ...
    title.value = '新增签到配置';
    state.form = { id: '', consecutiveDay: 1, rewardPoint: 1, sortOrder: 0, status: '0' };
  }
};
  ...
  <ElDialog v-model="visible" :title="title" width="500px" draggable>
    <ElForm ref="formRef" :model="state.form" :rules="rules" label-width="120px">
      <ElFormItem label="连续签到天数" prop="consecutiveDay">
        <ElInputNumber v-model="state.form.consecutiveDay" :min="1" style="width: 100%" />
      </ElFormItem>
      <ElFormItem label="奖励积分" prop="rewardPoint">
        <ElInputNumber v-model="state.form.rewardPoint" :min="1" style="width: 100%" />
      </ElFormItem>
