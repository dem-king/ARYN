...
const state = reactive({
  form: { id: '', ruleName: '', ruleType: '1', triggerScene: '', pointValue: 1, status: '0' },
});
...
const initForm = async (row?: any) => {
  ...
  if (row?.id) {
    ...
    title.value = '新增积分配置';
    state.form = { id: '', ruleName: '', ruleType: '1', triggerScene: '', pointValue: 1, status: '0' };
  }
};
  ...
  <ElDialog v-model="visible" :title="title" width="500px" draggable>
    <ElForm ref="formRef" :model="state.form" :rules="rules" label-width="100px">
      ...
      <ElFormItem label="触发场景" prop="triggerScene">
        <ElSelect v-model="state.form.triggerScene" placeholder="请选择触发场景" style="width: 100%">
          <ElOption v-for="item in sceneOptions" :key="item.value" :label="item.label" :value="item.value" />
        </ElSelect>
      </ElFormItem>
      <ElFormItem label="积分值" prop="pointValue">
        <ElInputNumber v-model="state.form.pointValue" :min="1" style="width: 100%" />
      </ElFormItem>
