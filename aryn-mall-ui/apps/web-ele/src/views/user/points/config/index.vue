  ...
  <div class="hx-layout-container">
    <div class="hx-layout-container-auto hx-layout-container-view">
      <div class="hx-table-toolbar">
        <div>
          <ElButton type="primary" v-access:code="'user:pointsconfig:add'" @click="handleAdd" :icon="Plus">新增</ElButton>
        </div>
      ...
      <ElTable v-loading="loading" :data="state.tableData" border>
        ...
        <ElTableColumn prop="ruleType" label="规则类型" width="100">
          <template #default="scope">
            ...
            <ElTag v-else-if="scope.row.ruleType === '2'" type="danger">消耗</ElTag>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="triggerScene" label="触发场景" width="100">
          <template #default="scope">{{ sceneMap[scope.row.triggerScene] || scope.row.triggerScene }}</template>
        </ElTableColumn>
        ...
        <ElTableColumn label="操作" width="150" align="center">
          <template #default="scope">
            <ElButton link type="primary" v-access:code="'user:pointsconfig:edit'" @click="handleEdit(scope.row)" :icon="Edit">编辑</ElButton>
            <ElButton link type="danger" v-access:code="'user:pointsconfig:del'" @click="handleDelete(scope.row)" :icon="Delete">删除</ElButton>
          </template>
        </ElTableColumn>
      </ElTable>
