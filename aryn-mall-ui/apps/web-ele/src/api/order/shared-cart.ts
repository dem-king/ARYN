...
export async function getSharedCartPage(query: any) {
  return requestClient.get('/order/shared-cart/page', { params: query });
}

export async function getSharedCartDetail(id: string) {
  return requestClient.get(`/order/shared-cart/${id}`);
}
...
export async function getPromotionSnapshots(orderId: string) {
  return requestClient.get('/order/promotion-snapshot/list', {
    params: { orderId },
  });
}
