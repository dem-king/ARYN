...
@Tag(description = "message-notice-staff", name = "工作人员通知收件箱")
public class StaffNoticeController {
	...
	public Result<NoticeInboxPageVO> inbox(NoticeInboxQuery query) {
		...
		return Result.success(noticeService.inbox(user.getTenantId(), MessageIdentityType.SYS_USER, user.getUserId(), query));
	}
	...
	public Result<NoticeInboxItemVO> detail(@PathVariable String id) {
		ArynUser user = currentUser();
		return Result.success(noticeService.inboxDetail(user.getTenantId(), MessageIdentityType.SYS_USER,
				user.getUserId(), id));
	}
	...
	public Result<Long> unreadCount() {
		ArynUser user = currentUser();
		return Result.success(noticeService.unreadCount(user.getTenantId(), MessageIdentityType.SYS_USER,
				user.getUserId()));
	}
	...
	public Result<Void> markRead(@PathVariable String id) {
		ArynUser user = currentUser();
		noticeService.markRead(user.getTenantId(), MessageIdentityType.SYS_USER, user.getUserId(), id);
	...
	public Result<Void> markAllRead() {
		ArynUser user = currentUser();
		noticeService.markAllRead(user.getTenantId(), MessageIdentityType.SYS_USER, user.getUserId());
		return Result.success();
	}
	...
	public Result<Void> hide(@PathVariable String id) {
		ArynUser user = currentUser();
		noticeService.hide(user.getTenantId(), MessageIdentityType.SYS_USER, user.getUserId(), id);
		return Result.success();
	}

	@PostMapping("/{id}/reply")
	@Operation(summary = "回复内部通知")
	public Result<ChatMessageVO> reply(@PathVariable String id, @Valid @RequestBody NoticeReplyRequest request) {
		ArynUser user = currentUser();
		return Result.success(noticeReplyService.replyAsStaff(user.getTenantId(), user.getUserId(), user.getNickname(),
