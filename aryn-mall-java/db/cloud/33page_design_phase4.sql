-- 商城装修定时/灰度发布与指标增量迁移（Cloud 微服务模式：表进 aryn_promotion，菜单进 aryn_upms，租户白名单进 aryn_nacos）
-- 目标库：aryn_promotion / aryn_upms / aryn_nacos
-- 特性：可重复执行，不执行 DROP/TRUNCATE，不覆盖已有业务数据。
-- 执行：mysql -u root -p aryn_promotion < 33page_design_phase4.sql；菜单与 Nacos 段自动切换目标库
...
-- 租户拦截表白名单登记 page_design_release_target（Cloud 模式配置位于 Nacos）
-- 说明：幂等执行；page_design_metric_daily 不进白名单（原生 upsert 与拦截器冲突），租户由服务端显式写入。
USE `aryn_nacos`;
...
SET @promotion_content = IF(
  ...
  NULL,
  IF(
    @promotion_content LIKE '%- page_design_release_target\n%',
    @promotion_content,
    REPLACE(
      @promotion_content,
      '      - page_design_theme\n',
