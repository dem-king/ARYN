...
--
-- 目标库：aryn_product（商品与船供资料）、aryn_vessel（船舶成员）；只读引用 aryn_user
...
--
-- 执行：mysql -u root -p < 62ship_supply_seed_acceptance.sql
...
DELETE FROM `aryn_vessel`.`vessel_bind_apply` WHERE `id` LIKE '969%';
DELETE FROM `aryn_user`.`user_info` WHERE `id` LIKE '97%';
DELETE FROM `aryn_vessel`.`vessel_member` WHERE `id` LIKE '962%';
DELETE FROM `aryn_vessel`.`vessel_call` WHERE `id` LIKE '968%';
...
-- ===========================================================================
INSERT INTO `aryn_vessel`.`vessel_member`
...
FROM `aryn_user`.`user_info` u WHERE u.`id` = '2040654277629796353' AND u.`del_flag` = '0';

INSERT INTO `aryn_vessel`.`vessel_member`
  (`id`, `vessel_id`, `user_id`, `member_role`, `can_edit`, `can_confirm`, `status`, `join_time`, `remark`, `tenant_id`, `create_by`, `create_time`, `del_flag`)
SELECT '9620000000000000002', '9610000000000000001', u.`id`, '3', '1', '1', '1', NOW(), '采购确认人（可核定数量并提交整船订单）', '1590229800633634816', 'seed', NOW(), '0'
FROM `aryn_user`.`user_info` u WHERE u.`id` = '2040656345832747009' AND u.`del_flag` = '0';

INSERT INTO `aryn_vessel`.`vessel_member`
  (`id`, `vessel_id`, `user_id`, `member_role`, `can_edit`, `can_confirm`, `status`, `join_time`, `remark`, `tenant_id`, `create_by`, `create_time`, `del_flag`)
SELECT '9620000000000000003', '9610000000000000001', u.`id`, '2', '1', '0', '1', NOW(), '普通船员（只能维护自己的明细）', '1590229800633634816', 'seed', NOW(), '0'
FROM `aryn_user`.`user_info` u WHERE u.`id` = '2096466699352522754' AND u.`del_flag` = '0';
...
INSERT IGNORE INTO `aryn_user`.`user_info`
...
INSERT INTO `aryn_vessel`.`vessel_bind_apply`
...
FROM `aryn_user`.`user_info` u
WHERE u.`id` = '9700000000000000001'
  AND u.`del_flag` = '0'
  AND NOT EXISTS (
    SELECT 1 FROM `aryn_vessel`.`vessel_member` m
...
-- ===========================================================================
INSERT INTO `aryn_vessel`.`vessel_call`
...
UNION ALL
SELECT '船舶成员(应 3)', COUNT(*) FROM `aryn_vessel`.`vessel_member` WHERE `del_flag` = '0' AND `id` LIKE '962%'
UNION ALL
SELECT '待审核绑定申请(应 >=0)', COUNT(*) FROM `aryn_vessel`.`vessel_bind_apply`
WHERE `del_flag` = '0' AND `id` LIKE '969%' AND `status` = '1'
UNION ALL
SELECT '可用靠港计划(应 >=2)', COUNT(*)
FROM `aryn_vessel`.`vessel_call`
