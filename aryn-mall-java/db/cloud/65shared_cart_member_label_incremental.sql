-- 悦航购共享采购按人拆行与配送到人标签增量迁移（Cloud 微服务模式）
--
-- 目标库：aryn_order（订单库）
-- 租户白名单：Cloud 侧在 aryn_order 的租户白名单配置（Nacos: aryn-order-biz-dev.yml）。
...
--
-- 执行：mysql -u root -p < 65shared_cart_member_label_incremental.sql
