...
function handleSearch() {
  ...
  const isPrevSearch = pages.length > 1 && pages[pages.length - 2]?.route === 'sub-pages/product/goods-search/index'
  if (isPrevSearch) {
    router.back()
  }
  else {
...
<template>
  <z-paging ref="pagingRef" v-model="state.goodsList" :auto="false" @query="queryList">
    <template #top>
      <hr-search-navbar
    ...
    <view class="mb-20rpx mt-10rpx">
      ...
      >
        <view
          ...
          :class="filterParam.recommend ? 'item-selected text-primary' : ''" @click="sortHandler('default')"
        >
          综合推荐
        </view>
        ...
        <view
          class="relative mt-1px flex flex-1 items-center justify-center text-center" :class="filterParam.newGoods ? 'item-selected text-primary' : ''"
      ...
      <view :class="layout ? 'p-10rpx' : 'p-0'">
        <view class="flex flex-wrap gap-10rpx">
          <view
            v-for="(item, index) in state.goodsList" :key="index"
            :class="layout ? 'w-[calc(50%-6rpx)]  bg-white rounded-16rpx shadow-sm' : 'flex w-full box-border p-10px bg-white shadow-sm'"
            @click="toDetail(item.id)"
          >
            <image
              :src="item.spuUrls[0]"
              :class="layout ? 'w-full h-160px block rounded-t-16rpx' : 'w-[calc(35%-10rpx)] h-120px block mr-20rpx rounded-8rpx flex-shrink-0'"
            />
            <view class="p-20rpx" :class="layout ? '' : 'flex-1'">
              <wd-text :lines="2" color="inherit" size="14px" :text="item.name" />
              ...
              <view class="flex items-center justify-between pt-10rpx">
                <wd-text :text="item.salesPrice" size="16px" mode="price" color="red" prefix="￥" />
                <view>
