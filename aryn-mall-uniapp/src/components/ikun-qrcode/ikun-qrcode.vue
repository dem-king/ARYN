...
<template>
  <view class="qr-wrapper" :style="{ width: `${width}${unit}`, height: `${height}${unit}` }">
    ...
    <view v-for="(row, rowI) in modules" v-else :key="rowI" class="qr-content">
      <view v-for="(col, colI) in row" :key="colI">
        <view
          v-if="col.isBlack" :style="{ width: `${per}${unit}`, height: `${per}${unit}` }"
