...
export default {
  ...
  computed: {
    ...
    isManyCom() {
      ...
      if (
        ...
        && goodsInfo[specListName][0].specsName === defaultSingleSkuName
      ) {
        isMany = false
      }
      return isMany
    ...
    priceCom() {
      ...
      if (selectShop[skuIdName]) {
        str = selectShop.salesPrice
      }
      else {
        str = goodsInfo.salesPrice
      }
      return str
    },
    // 库存显示
  ...
  methods: {
    ...
    updateGoodsInfo(goodsInfo) {
      ...
      // 更新选中sku的库存信息
      const select_sku_info = that.getListItem(that.goodsInfo[skuListName], that.skuIdName, that.selectShop[that.skuIdName])
    ...
    async open() {
      ...
      if (that.customAction && typeof that.customAction === 'function') {
        try {
          ...
          const { message = '' } = err
          if (message.includes('.catch is not a function')) {
    ...
    close(s) {
      ...
      if (s === 'mask') {
        if (that.maskCloseAble !== false) {
          that.$emit('input', false)
          that.$emit('close', that.selectArr)
          that.$emit('update:modelValue', false)
        }
      }
      else {
    ...
    skuClick(value, index1, index2) {
      const that = this
      if (value.ishow) {
        if (that.selectArr[index1] !== value.specsValueName) {
          that.$set(that.selectArr, index1, value.specsValueName)
    ...
    checkSelectShop() {
      ...
      if (that.selectArr.every(item => item !== '')) {
        ...
        that.selectPic = that.selectShop.picUrl
      }
      else {
        that.selectShop = {}
      }
    ...
    checkInpath(clickIndex) {
      ...
      for (let i = 0, len = specList.length; i < len; i++) {
        ...
        for (let j = 0; j < len2; j++) {
          ...
          that.$set(choosed_copy, i, specList[i].list[j].specsValueName)
          const choosed_copy2 = choosed_copy.filter(item => item !== '' && typeof item !== 'undefined')
    ...
    checkItem() {
      ...
      originalSkuList.map((skuItem, index) => {
        if (skuItem[stockName] > 0 && skuItem.status == '0') {
      ...
      const result = skuList.reduce(
        (arrs, items) => {
          const specsValueNames = items.specsArr.map((item) => {
            return item.specsValueName
          })
          return arrs.concat(
            specsValueNames.reduce(
              (arr, item) => {
                return arr.concat(
                  arr.map((item2) => {
    ...
    addCart() {
      ...
      that.checkSelectComplete({
        success(selectShop) {
          const data = {
            spuId: selectShop.spuId,
            quantity: that.selectNum,
            spuName: that.goodsInfo.name,
            picUrl: that.selectPic ? that.selectPic : that.goodsInfo.spuUrls[0],
            skuId: selectShop.id,
            salesPrice: selectShop.salesPrice,
            specsInfo: that.selectArr.join('；') !== '默认' ? that.selectArr.join('；') : '',
          }
          that.$emit('add-cart', data)
        },
      })
    },
    // 立即购买
    ...
    autoClickSku() {
      ...
      if (specListArr.length === 1) {
        ...
        for (let i = 0; i < specList.length; i++) {
          const sku = that.getListItem(skuList, that.skuArrName, [
            {
              specsValueName: specList[i].specsValueName,
            },
          ])
    ...
    defaultSelectSku() {
      ...
      if (defaultSelect && defaultSelect.specsArr && defaultSelect.specsArr.length > 0) {
    ...
    selectSku(obj = {}) {
      const that = this
      const { specsArr: skuArr, quantity: selectNum } = obj
      const specListArr = that.goodsInfo[that.specListName]
      ...
      if (skuArr && specListArr.length === skuArr.length) {
        ...
        for (let index = 0; index < skuArr.length; index++) {
          const specsValueName = skuArr[index].specsValueName
          const specList = specListArr[index].list

          const index1 = index
          const index2 = that.getListIndex(specList, 'specsValueName', specsValueName)
          if (index2 === -1) {
            clickKey = false
            break
          }
    ...
    previewImage() {
      const that = this
      const { selectPic, goodsInfo, goodsThumbName } = that
      const spuUrls = goodsInfo[goodsThumbName]
      const src = selectPic || spuUrls[0]
      if (src) {
        uni.previewImage({
          urls: [src],
        })
...
<template>
  <view
    class="vk-data-goods-sku-popup" catchtouchmove="true" :class="valueCom && complete ? 'show' : 'none'"
  ...
  >
    ...
    <view class="layer attr-content" :style="{ borderRadius: `${borderRadius}rpx ${borderRadius}rpx 0 0` }">
        ...
        <scroll-view class="specification-wrapper-content" scroll-y="true">
          <view class="specification-header">
            <view class="specification-left">
              <image
                class="product-img" :src="selectPic ? selectPic : goodsInfo.spuUrls ? goodsInfo.spuUrls[0] : ''"
                :style="{ backgroundColor: goodsThumbBackgroundColor }" mode="aspectFill" @click="previewImage"
              />
            </view>
            <view class="specification-right">
              <view class="price-content text-primary!">
                <text class="sign">
                  ¥
                </text>
                <text class="price">
              ...
              <view v-if="!hideStock && selectShop.stock" class="inventory">
                {{ stockText }}：{{ stockCom }}
              </view>
              <view v-else class="inventory" />
              <view v-show="isManyCom" class="choose text-14px!">
                已选：{{ selectArr.join(' ') }}
              </view>
            </view>
          </view>
          ...
          <view class="specification-content">
            <view
              v-for="(item, index1) in goodsInfo[specListName]" v-show="isManyCom" :key="index1"
            ...
            >
              ...
              <view class="item-wrapper">
                <view
                  v-for="(item_value, index2) in item.list" :key="index2" class="item-content"
                  :class="[subIndex[index1] === index2 ? 'actived border-primary! bg-primary! text-white!' : '']" :style="[
            ...
            <view class="number-box-view">
              ...
              <view style="flex: 4; text-align: right">
                <vk-data-input-number-box
                  v-model="selectNum" :min="minBuyNum || 1" :max="maxBuyNumCom"
      ...
      <view
        v-if="outFoStock || mode === 4" class="btn-wrapper"
      ...
      <view v-else-if="mode === 1" class="btn-wrapper" :class="{ 'safe-area-inset-bottom': safeAreaInsetBottom }">
        <view class="sure add-cart bg-secondary!" style="border-radius: 38rpx 0rpx 0rpx 38rpx" @click="addCart">
          {{ addCartText }}
        </view>

        <view class="sure bg-primary!" style="border-radius: 0rpx 38rpx 38rpx 0rpx" @click="buyNow">
          {{ buyNowText }}
        </view>
      </view>
      <view v-else-if="mode === 2" class="btn-wrapper" :class="{ 'safe-area-inset-bottom': safeAreaInsetBottom }">
        <view class="sure add-cart bg-secondary!" @click="addCart">
          {{ addCartText }}
        </view>
      </view>
      <view v-else-if="mode === 3" class="btn-wrapper" :class="{ 'safe-area-inset-bottom': safeAreaInsetBottom }">
        <view class="sure bg-primary!" @click="buyNow">
          {{ buyNowText }}
        </view>
      </view>
    </view>
